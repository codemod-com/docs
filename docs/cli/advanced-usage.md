...
If you want to use Intutia through a graphical interface, check out the [Codemod.com VS Code Extension](/docs/vs-code-extension/quickstart).
...
After running this command, if any git diff exists, Codemod.com will use the diff as before/after snippets in [Codemod Studio](https://codemod.studio). This command uses the following format:
...
:::tip
You can specify your glob patterns to include specific file formats or directory structures. The default pattern Codemod.com CLI uses is: `--include "**/*.*{ts,tsx,js,jsx,mjs,cjs,mdx}"`.
:::
...
:::tip
By default, the Codemod.com CLI excludes the following glob pattern: `--exclude "**/node_modules/**/*.*"`. If you are using a different glob pattern, we recommend excluding the `node_modules` directory to avoid unnecessary codemod runs.
:::

### `--targetPath`

The `--targetPath` option can be used to specify the directory of your project that Codemod.com should target while running codemods.
...
The `--useJson` option can be used to switch Codemod.com's CLI responses to JSON format.
...
The `--threadCount` option can be used to specify the number of worker threads Codemod.com uses while running codemods. This option is set to 4 threads by default.
