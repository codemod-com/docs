...
This is why we created the Intuita platform to build, test, distribute, and execute codmods for codebases of any size.
...
* **Share**: Add your codemods to Intuita's private or public distributed registries, making them available to colleagues or the community.
* **Run**: Discover and run codemods with a single click or CLI command, tweak changes as needed, and iteratively improve them by providing feedback to Intuita AI or the community.
* **Manage**: Codemods alone are not sufficient for successful migrations in large codebases and teams. Orchestrating the changes made by codemods across multiple teams requires a platform that can integrate with the existing tools of each software team. These features, which are only needed by large teams, fall under Intuita's enterprise offerings.
...
We also built codemod `mocha/vitest/migrate-configurations` using Intuita's engine (called filemod) as jscodeshift does not allow creating and removing files.
...
Codemods could be run via Intuita's VS Code Extension ([click on this deep-link](https://tinyurl.com/ytpbx5kb)), or via the CLI command from the root folder of the project:
```bash
...
The first two had a handful of Mocha.js test suites and we migrated them first to spot problems early in the development. Once the codemod was ready and accurate, we merged them into the Codemod Registry. Once codemods are published to the registry, they automatically become accessible to the community via Intuita VSCode Extension or the Intuita CLI.
...
Since the Intuita CLI can execute local codemods, we used it to see if the changes to the codemod resulted in the correct transformation of the codebase in question. This speeded up the development process tremendously.
...
3. **OSS**: With Intuita's open-source platform we can automate crucial yet tedious tasks for ourselves, colleagues, and the community.

If you, just like us, are passioante about the future of software development, join our [community](https://go.intuita.io/slack) or [team](https://intuita.notion.site/Join-Intuita-b9c9bf82d38341f8936744b6aac20dba).
